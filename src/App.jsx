...
function App() {
  ...
  const getBeers = async () => {
    let allBeers = [];
    for (let i = 1; i < 6; i++) {
      const beersArr = await fetch(
        `https://api.punkapi.com/v2/beers?per_page=65&page=${i}`
      )
  ...
  function filterArray(beers, filters) {
    const filterConditions = Object.keys(filters);
    const checkboxFilter = beers.filter((item) => {
      // validates all filter criteria
      return filterConditions.every((key) => {
        // ignores non-function predicates
        if (typeof filters[key] !== "function") return true;
        return filters[key](item[key]);
      });
    });
    return checkboxFilter.filter((beer) =>
      beer.name.toLowerCase().includes(searchCriteria.toLowerCase())
    );
  }
